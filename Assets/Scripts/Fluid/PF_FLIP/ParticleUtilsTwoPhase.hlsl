#ifndef PARTICLE_UTILS_TWOPHASE_HLSL
#define PARTICLE_UTILS_TWOPHASE_HLSL

// Copy of GPU_FLIP/ParticleUtils.hlsl for the two-phase PF_FLIP renderer:
// identical ray-sphere billboard pipeline, except the phase decode (w < 0
// = air) additionally halves the drawn radius so the air band reads
// visually lighter than the water band. Keep in sync with the original.
...
struct g2f
{
    centroid noperspective linear float4 vertex : SV_POSITION;
...
inline float MyRemap(float x, float in_min, float in_max, float out_min, float out_max)
{
...
void Geometry(point v2g p[1], inout TriangleStream<g2f> stream)
{
    ...
    // w carries speed with a phase sign in two-phase sims: positive =
    // water, negative = air. Air particles draw at half radius.
    const bool is_air = p[0].particle_data.w < 0;
    if (is_air) return;
    const float sphere_radius = is_air ? _Radius * 0.5 : _Radius;
    ...
    const float speed = abs(p[0].particle_data.w);
    const float remap_speed = MyRemap(speed, _VelocityRange.x, _VelocityRange.y, 0, 1);
    const float4 color = is_air
        ? float4(0.75, 0.78, 0.82, 1) // air phase: pale mist
        : lerp(_SlowColor, _FastColor, remap_speed);
...
half SampleOcclusion(float2 screenUV)
{
    #if defined(_SCREEN_SPACE_OCCLUSION)
        return saturate(SampleAmbientOcclusion(screenUV) + (1.0 - _AmbientOcclusionParam.x));
    #else
        return half(1.0);
    #endif
}
...
float4 Fragment(g2f i, out float depth_test : SV_DepthLessEqual) : SV_Target
{
    ...
    const float3 camera_space_view_dir = normalize(-camera_space_pos - float3(2, 5, 0));

    const float vdotn = dot(camera_space_view_dir, camera_space_normal);
    const float fresnel = 1 + (1.0 - _FresnelPower) * pow(1.0 - vdotn, 5) / _FresnelPower;

    float4 clipPos = TransformWViewToHClip(i.camera_space_sphere_center_pos);
    clipPos.y = -clipPos.y;

    float ao = SampleOcclusion(GetNormalizedScreenSpaceUV(i.vertex));

    color.rgb *= lerp(fresnel, float3(0, 0.05, 0.1), (1 - ao));
...
// Fragment Shader
// --------------------------------------------------------------------
float4 FragDepth(g2f i, out float depth_test : SV_DepthLessEqual) : SV_Target
{
    const float3 m = normalize(i.camera_space_pos);
    const float3 minus_a = -i.camera_space_sphere_center_pos;
    const float dot_m_minus_a = dot(m, minus_a);
...
#endif // PARTICLE_UTILS_TWOPHASE_HLSL
