...
// Per-particle flags live in the spare high bits of packedPosition.y
// (PackUint3 only uses bits 0-29):
//   bit 30 = phase    (1 water / 0 air)
//   bit 31 = ballistic (set by G2P for spray droplets, consumed by Advection)
inline uint UnpackPhase(uint v)
{
    return (v >> 30) & 1u;
}

inline uint UnpackBallistic(uint v)
{
    return (v >> 31) & 1u;
}

// Phase/ballistic-preserving repack: Advection must route every moving
// particle through this instead of EncodePosition, which would wipe the
// flags every frame.
inline uint2 EncodePositionWithFlags(float3 pos, uint phase, uint ballistic)
{
    float3 cellPos = pos * _InvCellSize;
    return uint2(Morton3DEncode((uint3)floor(cellPos)),
                 (ballistic << 31) | (phase << 30) | PackUint3(round(frac(cellPos) * 1023)));
}

inline uint2 EncodePositionAndPhase(float3 pos, uint phase)
{
    return EncodePositionWithFlags(pos, phase, 0);
}

inline uint2 EncodePosition(float3 pos)
{
    return EncodePositionWithFlags(pos, 0, 0);
}

// ---------------------------------------------------------------
// Per-cell phase info (RInt _GridPhase): low 8 bits = water particle
// count (spray detection), bit 8 = water-cell flag. Empty cells get
// the flag from the previous SDF sign (see ClassifyGridPhase).
// ---------------------------------------------------------------
#define GRIDPHASE_WATER_FLAG (1u << 8)

inline uint CellWaterCount(uint phaseInfo)
{
    return phaseInfo & 255u;
}

inline uint CellIsWater(uint phaseInfo)
{
    return (phaseInfo >> 8) & 1u;
}

// ---------------------------------------------------------------
// Two-phase face mobility, harmonic mean of 1/rho scaled by the water
// density: water-water 1, air-air 1000, mixed ~2. The uniform scaling
// keeps the solution invariant but lifts the water diagonal (~0.006
// unscaled) out of the half-float denormal range after 4 levels of
// 0.125x coefficient aggregation.
// ---------------------------------------------------------------
inline float FaceMobility(uint waterA, uint waterB)
{
    const float rhoA = waterA ? 1000.0 : 1.0;
    const float rhoB = waterB ? 1000.0 : 1.0;
    return 2.0 * 1000.0 / (rhoA + rhoB);
}
