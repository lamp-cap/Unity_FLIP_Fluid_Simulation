...
namespace Abecombe.GPUBufferOperators
{
    public class GPUPrefixScan : IDisposable
    {
        ...
        private void Scan(GraphicsBuffer dataBuffer, GraphicsBuffer totalSumBuffer, uint bufferOffset, bool returnTotalSum, int bufferIndex)
        {
            ...
            const int numGroupThreads = 512;
            const int numElementsPerGroup = 4 * numGroupThreads;
            ...
            {
                Debug.Assert(_groupSumBufferList.Count >= bufferIndex, "Fatal Error in Prefix Scan");
                return;
            }
        ...
        public void Dispose()
        {
            if (_groupSumBufferList != null) { _groupSumBufferList.ForEach(x => x.Release()); _groupSumBufferList = null; }
            if (_totalSumBuffer != null) { _totalSumBuffer.Release(); _totalSumBuffer = null; }
        }
    }
}
