...
{
    public partial class FLIPWithSurfaceTurbulence : MonoBehaviour
    {
        ...
        // Which surface-detail system rides on top of the base solver this
        // frame. Mutually exclusive by construction: they all fight for the
        // same frame budget, and the two mesh displacers both rewrite the MC
        // vertex buffer. Each system's Step/Apply self-gates on the frozen
        // copy below, so the frame flow can call them unconditionally.
        public enum SurfaceDetailMode
        {
            None,
            SurfaceTurbulence, // Mercier et al. 2015 point cloud
            BandWaves,         // damped wave eq. on the SDF band
            WaveCurves,        // Skrivan et al. 2019 Lagrangian wave packets
        }
        ...
        public bool drawBaseParticles = false;
        public DrawType drawType;

        [Header("Surface Detail")]
        [Tooltip("Which surface-detail system runs on top of the base solver. Replaces the old enableSurfaceTurbulence/enableBandWaves/enableWaveCurves bools.")]
        public SurfaceDetailMode surfaceDetail = SurfaceDetailMode.SurfaceTurbulence;

        // Frozen copy of surfaceDetail, taken once in Start. The active path
        // is fixed for the whole run: runtime edits to surfaceDetail are
        // ignored, and only the selected system ever initializes.
        private SurfaceDetailMode _surfaceDetail = SurfaceDetailMode.None;
        ...
        // UpdateVelocity in PressureProjection, plus the NB-side
        // ResampleParticles / JFA / CountingSort / GridToParticle / BandWaves)
        // includes NBFlipUtils.hlsl, i.e. [numthreads(8,8,8)] —
        // (GridSize+7)/8 covers the domain exactly once. Do NOT widen these
        // dispatches: UpdateVelocity is an in-place read-modify-write of
        // _VelocityW, so over-dispatch is a data race that applies the
        // pressure gradient several times per cell and kills flow.
        ...
        void Start()
        {
            ...
            // Freeze the mode: every runtime gate reads _surfaceDetail, so
            // Inspector edits mid-play change nothing until the next run.
            _surfaceDetail = surfaceDetail;

            if (_surfaceDetail == SurfaceDetailMode.SurfaceTurbulence)
                InitSurfaceTurbulence();

            // Cheap SDF-band alternative to the surface turbulence point cloud
            // (see FLIPWithSurfaceTurbulence.BandWaves.cs). The compute shader
            // must be assigned on the component.
            if (_surfaceDetail == SurfaceDetailMode.BandWaves)
                InitBandWaves();

            // Lagrangian wave packets on the free surface (Skrivan et al.
            // 2019, see FLIPWithSurfaceTurbulence.WaveCurves.cs).
            if (_surfaceDetail == SurfaceDetailMode.WaveCurves)
                InitWaveCurves();
        ...
        private void OnGUI()
        {
            ...
            // Live count is GPU-resident; this is a ~1 Hz poll purely for display
            // (PollSurfaceDiagnostics). Nothing dispatches off it.
            if (_surfaceDetail == SurfaceDetailMode.SurfaceTurbulence)
                GUI.Label(new Rect(10, 50, 700, 20),
                    $"Surface points: ~{_liveCountApprox} / {maxSurfacePoints}" +
                    (_overflowTotal > 0 ? $"  (dropped {_overflowTotal})" : ""), _labelStyle);

            if (_surfaceDetail == SurfaceDetailMode.WaveCurves)
                GUI.Label(new Rect(10, 50, 700, 20), _wcStatsText, _labelStyle);

            // Band-wave diagnostics, refreshed by the same gated 1 Hz poll.
            if (_surfaceDetail == SurfaceDetailMode.BandWaves)
                GUI.Label(new Rect(10, 90, 700, 20), _bandWaveStatsText, _labelStyle);
        }
        ...
        private void Simulation()
        {
            ...
            // try/finally so an exception cannot leak the CommandBuffer. Without it a
            // throw anywhere below skips Release, and the pool leaks one buffer per
            // frame until VRAM runs out — which surfaces as an unrelated-looking
            // "CreateCommittedResource failed (887a0005)" and a hard crash, tens of
            // seconds after the actual error.
            try
            {
                cmd.Clear();
                ...
                if (!_pause)
                {
                    ...
                    // Runs on the post-advection coarse positions while
                    // _gridParticleRange still describes the pre-advection layout the
                    // _particlesPrev snapshot was taken in.
                    SurfaceTurbulenceStep(cmd);
                ...
                // swap slots: this frame's write buffer/count becomes the
                // read side that Update's DrawWaveCurves renders
                SwapWaveCurves();
        ...
        private void InitParticles()
        {
            ...
            // Both init blocks sit flush in opposite floor corners rather than
            // floating. A suspended block exposes all six faces, and InitSurfacePoints
            // seeds every one of them — so the surface point count peaked at startup
            // with the bottom and side faces, which a settled liquid never needs.
            //
            // Extent is fixed by the Morton fill: half of ParticlesBufferSize reaches
            // coord 127 on each axis, so 127 * GridSpacing * _Scale = 17.78 world
            // units per side. Cell 0 is the wall (ResampleParticles.compute GetType
            // returns SOLID out of bounds), so the first fluid cell is at GridSpacing.
        ...
        private void GridToParticle(CommandBuffer cmd)
        {
            ...
            // surface advection needs (curPos - prevPos) per coarse particle
            SnapshotCoarseParticles(cmd);
        ...
        private void PrepareForRenderingMesh(CommandBuffer cmd)
        {
            ...
            // Ripple displacement on the freshly built vertices. WaveCurves
            // no longer participates here — it draws its own ribbons in
            // Update (DrawWaveCurves) over the undisplaced mesh. The pass
            // self-gates on surfaceDetail (and bandWaveDisplaceMesh).
            BandWaveDisplaceMeshPass(cmd);
        }
        ...
        private void OnDestroy()
        {
            ...
            // Unconditional: DisposeSurfaceTurbulence self-guards on _surfaceInited,
            // so this stays correct even if surfaceDetail changes between runs
            // after a successful init (or cleared by the memory guard).
            DisposeSurfaceTurbulence();

            // Same self-guarding pattern for the band-wave buffers.
            DisposeBandWaves();

            // And the wave-curve buffers/material (~38 MB at default sizes) —
            // without this every play session leaks them (native GPU memory,
            // reclaimed only by domain reload).
            DisposeWaveCurves();
